...
/**
 * @brief AuthReq (Authentication requirements) 认证要求。表示设备是否需要绑定，是否需要支持防止中间人攻击，是否支持安全配对等
 * 
 * @details 
 * @code 一些涉及到的专业名词或缩写:
 * - LESC (LE secure connections): 又称 SC，蓝牙 4.2 引入的一种新的密钥生成方式和验证方式，
 ...
 *      如果采用 passkey 配对方式，TK 就是 passkey；
 *      如果采用 OOB 配对方式，TK 就是 OOB 里面的信息。
 * - STK (short term key ，短期密钥): legacy 配对里面的概念，STK 是通过 TK 推导出来的，通过 TK 对设备 A 和 B 的随机数进行加密，即得到 STK。
 ...
 *      从而得到 LTK ， LTK 由设备 A 和 B 各自同时生成，因此 LTK 不会出现在 LESC 蓝牙空中包中，大大提高了蓝牙通信的安全性。
 ...
 * @endcode
 * 
 * @code 认证要求标志 (Authentication requirements flags)
 * LSB                                                                  MSB
 *  ┌───────────────┬─────────┬─────────┬──────────┬─────────┬───────────┐
 ...
 * 
 */
typedef uint8_t xf_ble_sm_authen_rqmt_t;         /*!< combination of the above bit pattern */

#define XF_BLE_SM_AUTHEN_RQMT_NONE          (0)         /*!< 0 No authentication requirements */
#define XF_BLE_SM_AUTHEN_RQMT_BOND          (1 << 0)    /*!< 1 << 0 device in the bonding with peer */
#define XF_BLE_SM_AUTHEN_RQMT_MITM          (1 << 2)    /*!< 1 << 2 MITM (防) 中间人攻击 */
#define XF_BLE_SM_AUTHEN_RQMT_SC            (1 << 3)    /*!< 1 << 3 安全连接 SC secure connection */
#define XF_BLE_SM_AUTHEN_RQMT_BOND_MITM     (XF_BLE_SM_AUTHEN_RQMT_BOND | XF_BLE_SM_AUTHEN_RQMT_MITM)     /*!< 0101 绑定，带 MITM (防) 中间人攻击 */
#define XF_BLE_SM_AUTHEN_RQMT_BOND_SC       (XF_BLE_SM_AUTHEN_RQMT_BOND | XF_BLE_SM_AUTHEN_RQMT_SC)       /*!< 1001 安全连接，带绑定 */
#define XF_BLE_SM_AUTHEN_RQMT_MITM_SC       (XF_BLE_SM_AUTHEN_RQMT_MITM | XF_BLE_SM_AUTHEN_RQMT_SC)       /*!< 1100 安全连接，带 MITM */
#define XF_BLE_SM_AUTHEN_RQMT_BOND_MITM_SC  (XF_BLE_SM_AUTHEN_RQMT_BOND | XF_BLE_SM_AUTHEN_RQMT_MITM | XF_BLE_SM_AUTHEN_RQMT_SC)   /*!< 1101 安全连接，带绑定及 MITM */
 ...
 *      - 手机上表现: 仅需点两次配对确认即可
 * - Passkey Entry ：这种方式是由两端设备分别输入 (单端或双端) 配对码的方式进行认证的。
 *      - 是蓝牙最早最传统的一种认证方式，有两种操作方法：
...
#define XF_BLE_SM_IO_CAP_OUT_DISP_ONLY          0   /*!< SIG: DisplayOnly = No input + Numeric Output
                                                     * 详情浏览本类型的注释中 "@details IO 能力详情" 的部分 */
#define XF_BLE_SM_IO_CAP_IO_DISP_Y_N            1   /*!< SIG: DisplayYesNo = ( (Yes / No) / (Keyboard) ) + Numeric Output
                                                     * 详情浏览本类型的注释中 "@details IO 能力详情" 的部分 */
#define XF_BLE_SM_IO_CAP_IN_KB_ONLY             2   /*!< SIG: KeyboardOnly = Keyboard + No Output 
                                                     * 详情浏览本类型的注释中 "@details IO 能力详情" 的部分 */
#define XF_BLE_SM_IO_CAP_NONE                   3   /*!< SIG: NoInputNoOutput = ( (No input) / (Yes / No) ) + No Output
                                                     * 详情浏览本类型的注释中 "@details IO 能力详情" 的部分 */
#define XF_BLE_SM_IO_CAP_KB_DISP                4   /*!< SIG: KeyboardDisplay 
 ...
 */
typedef uint8_t xf_ble_sm_oob_data_flag_t;  
#define XF_BLE_SM_OOB_DATA_NONE         0       /*!< 禁用 OOB，OOB 认证数据不存在 */
#define XF_BLE_SM_OOB_DATA_PRESENT      1       /*!< 启用 OOB，OOB 认证数据存在于远端设备 */

/**
 * @brief BLE SM 安全要求 (secty requirement)
 * 
 */
typedef uint8_t xf_ble_sm_secty_rqmt_t;
enum _xf_ble_sm_secty_rqmt_t {
    XF_BLE_SM_SECTY_M1L1_NONE,                       /*!< 安全模式 1 等级 1 : 无安全性 (无需认证且无需加密) */
    XF_BLE_SM_SECTY_M1L2_UNAUTHEN_ENCRYPT_PAIR,      /*!< 安全模式 1 等级 2 : 未经认证的加密配对 */
    XF_BLE_SM_SECTY_M1L3_AUTHEN_ENCRYPT_PAIR,        /*!< 安全模式 1 等级 3 : 经过认证的加密配对 */
    XF_BLE_SM_SECTY_M1L4_SECURE_CONN_ENCRYPT_PAIR,   /*!< 安全模式 1 等级 4 : 经过认证的安全连接 (LE Secure Connections)
                                                         * 与使用密钥为128位的加密配对 */
    XF_BLE_SM_SECTY_M2L1_UNAUTHEN_DATA_SIGN_PAIR,    /*!< 安全模式 2 等级 1 : 经过认证的数据签名配对 */
    XF_BLE_SM_SECTY_M2L2_AUTHEN_DATA_SIGN_PAIR,      /*!< 安全模式 2 等级 2 : 未经认证的数据签名配对 */
};

#define XF_BLE_SM_KEY_SIZE_MAX   (16)

typedef struct _xf_ble_sm_tk_t {
    /// Key value MSB -> LSB?
    uint8_t tk[XF_BLE_SM_KEY_SIZE_MAX];
} xf_ble_sm_tk_t;

typedef struct _xf_ble_sm_csrk_t {
    /// Key value MSB -> LSB?
    uint8_t tk[XF_BLE_SM_KEY_SIZE_MAX];
} xf_ble_sm_csrk_t;
